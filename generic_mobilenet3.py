...
def create_model(num_classes):
    ...
    custom_classifier = torch.nn.Linear(num_features, num_classes)
    model.classifier[-1] = custom_classifier
    return model
...
def evaluate2(test_dataset_embedding, test_dataset_class, train_dataset_embedding, train_class, output_results_path):
    classes = test_dataset_class
    cls2idx = {}
    ...
    silhouette_score_train = silhouette_score(train_dataset_embedding, train_class)
    silhouette_score_test = silhouette_score(test_dataset_embedding, test_class)
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Train and evaluate MobileNet model on custom dataset")
